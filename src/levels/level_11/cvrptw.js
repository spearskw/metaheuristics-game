 ...
 * @param {{ vehicleCapacity: number, customers: Array }} instance
 * @param {Float64Array[]} dist
 * @returns {{ capacityPenalty: number, twPenalty: number }}
 */
export function routePenalties(route, instance, dist) {
  if (route.length === 0) return { capacityPenalty: 0, twPenalty: 0 };
  ...
  const capacityPenalty = Math.max(0, totalDemand - vehicleCapacity) * CAPACITY_PENALTY_WEIGHT;
  ...
  for (const cid of route) {
    ...
    time += dist[prev][cid];
    if (time > c.dueDate) {
      twPenalty += (time - c.dueDate) * TW_PENALTY_WEIGHT;
    }
    if (time < c.readyTime) {
      time = c.readyTime;
    }
  ...
  // Return to depot
  time += dist[prev][0];
  if (time > customers[0].dueDate) {
    twPenalty += (time - customers[0].dueDate) * TW_PENALTY_WEIGHT;
  }

  return { capacityPenalty, twPenalty };
}

/**
 * Compute total cost for a solution: distance + capacity penalties + TW penalties.
 * @param {number[][]} routes
 * @param {{ vehicleCapacity: number, customers: Array }} instance
 * @param {Float64Array[]} dist
 * @returns {{ distance: number, capacityPenalty: number, twPenalty: number, total: number }}
 */
export function totalCost(routes, instance, dist) {
  let distance = 0;
  let capacityPenalty = 0;
  let twPenalty = 0;
  for (const route of routes) {
    distance += routeDistance(route, dist);
    const p = routePenalties(route, instance, dist);
    capacityPenalty += p.capacityPenalty;
    twPenalty += p.twPenalty;
  }
  return { distance, capacityPenalty, twPenalty, total: distance + capacityPenalty + twPenalty };
}
